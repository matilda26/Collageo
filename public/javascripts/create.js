...
var stage = new Konva.Stage({container: 'container', width: width, height: height, id: 'main'});
...
document.querySelector('#add').addEventListener('click', function() {
  for (var i = 0; i < selectedImages.length; i++) {
    ...
    var newImgGroup = new Konva.Group({x: 20, y: 110, draggable: true});
...
document.querySelector('#add-rect').addEventListener('click', function() {
  var rect = new Konva.Rect({width: 200, height: 150, fill: colors[0], name: 'rect'});
  var rectGroup = new Konva.Group({x: 100, y: 60, draggable: true, name: 'red-rect'});
  layer.add(rectGroup);
...
video.addEventListener('play', function () {
  i = window.setInterval(function () {
    ctx.drawImage(video, 20, 20, 260, 125)
  }, 20);
}, false);
